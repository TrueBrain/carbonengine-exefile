...
class CarbonBuildWindows(buildName: String, configType: String, preset: String) : BuildType({
    ...
    vcs {
        root(AbsoluteId("Carbon_ExeFile_2_ExefileFeatureKotlin"),"+:. => %github_checkout_folder%")
    ...
    triggers {
        vcs {
            triggerRules = "+:root=${AbsoluteId("Carbon_ExeFile_2_ExefileFeatureKotlin").id}:."
    ...
    features {
        pullRequests {
            vcsRootExtId = "${AbsoluteId("Carbon_ExeFile_2_ExefileFeatureKotlin")}"
            provider = github {
                authType = token {
